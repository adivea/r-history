...
interview respondents' walls (`animals`):
...
The quotes around "horse", etc. are essential here. Without the quotes R
will assume there are objects called `horse`, `duck` and `canary`. As these objects
...
```{r, results="show", purl=FALSE}
animals <- c("horse", "duck", "canary", "elephant")
animals[2]
animals[c(3, 2)]
```
...
```{r, results="show", purl=FALSE}
years > 5    # will return logicals with TRUE for the indices that meet the condition
...
1. Using this vector of years, create a new vector with the NAs removed.

```r
years <- c(1, 2, 1, 1, NA, 3, 1, 3, 2, 1, 1, 8, 3, 1, NA, 1)
```

2. Use the function `median()` to calculate the median of the `years` vector.

3. Use R to figure out how many elements of the vector are greater than 2.
...
# 3.
years_above_2 <- years_no_na[years_no_na > 2]
length(years_above_2)
```
...
structures, we are ready to start working with the SAFI or kings dataset and learn about data frames.
