...
library(tidyverse)
library(here)
kings <- read_csv(here("data/kings.csv"), na = "NULL", n_max = 54)
```
````

```{block, opts.label = "alerts" }
## Important Note!
...
kings %>%
  ...
  summarize(mean_reign = mean(Reign_duration)) %>%
  knitr::kable(caption = "We can also add a caption.", 
               col.names = c("House", "Gender", 
                             "Mean Years of Rule"))
```
...
we're interested in presenting the average reign duration in a royal house.

```{r in-line-code, purl=FALSE}
# create a summary data frame with the mean reign duration by royal house
mean_house <- kings %>%
    filter(!is.na(Start_year)) %>% 
    mutate(Reign_duration = End_year-Start_year) %>% 
    group_by(House) %>%
...
# and select the house we want to use
mean_Gorm <- mean_house %>%
  filter(House == "Gorm")
```

Now we can make an informative statement on the means of each House, and include
the mean values as in-line R-code. For example:

The mean reign duration can be
``` ``r "r round(mean_Gorm$mean_reign, 1)"`` ```

becomes...

The average reign duration in the House of Gorm is
`r round(mean_Gorm$mean_reign, 1)`.
...
and a little less boring. We will use the `kings_plotting` data from the
previous episode.

If you were unable to complete the previous lesson or did not save the data,
then you can create it in a new code chunk.

```{r plotting-data, purl=FALSE, eval=TRUE}
kings_plotting <- read_csv("data_output/kings_plotting.csv", n_max = 54)
```
...
`kings_plotting` data frame.


```{r my-fancy-plot, answer=TRUE, purl=FALSE, solution = TRUE}
kings_plotting %>% 
...
```{r caption-plot, eval=TRUE, fig.cap="I made this plot while attending an awesome Data Carpentries workshop where I learned a ton of cool stuff!", echo = FALSE}
...
## Inserting citations into an R Markdown file
