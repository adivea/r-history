---
title: "Textmining The Arabian Nights"
output: html_document
author: Adela Sobotkova & Laura Bang Jensen
date: "2023-08-24"
---

This script combines code I shared with students in Digital Methods course with that of Laura Jensen used to analyse the corpus of Arabian Nights in her 2022 text-mining project. 
...
In the dataframe there is a lot of text, that aren't part of the stories that are Arabian Nights. To get the most accurate result we will trim most of the excessive text away by specific rows:
```{r trim dataframe}
...
Let's figure out the frequency of words in Arabian Nights. The first thing we have to do is to tokenize the dataframe, so that every word gets it's own row in a dataframe:
...
### Wordcloud
Let's now make a wordcloud with the most frequent words in Arabian Nights. The first thing we want to do is to filter out any numbers from the text:
...
```
By using this code we can see that there are over 33.000 unique words in Arabian Nights. That is too many to fit into one wordcloud, so we want to limit the number of words in the wordcloud to the most frequent100.
...
Now we make a wordcloud based on the new dataframe, we just created, with only the 100 most frequent words in Arabian Nights:
```{r wordcloud}
...
### Sentiment Analysis
First we'll do a sentiment analysis on Arabian Nights. We focus on the ten main emotions in the "nrc" lexicon, which we have to load first:
```{r lexicon}
...
First, we want to split the book up into smaller sections, to get the most precise results, when we look for where in Arabian Nights we can find a high frequency of words.
...
### If a word appears
It would also be interesting to have a look at the word "sex". Does it even appear in Arabian Nights?:
```{r sex}
tidy_stories %>% 
